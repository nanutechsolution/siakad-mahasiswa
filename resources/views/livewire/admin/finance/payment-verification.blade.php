<div>
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                        ...
                        <td class="px-6 py-4">
                            <div class="font-bold text-slate-900 dark:text-white">
                                {{ $pay->billing->student->user->name }}</div>
                            <div class="text-xs text-slate-500 font-mono">{{ $pay->billing->student->nim }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-xs font-bold text-slate-700 dark:text-slate-300">{{ $pay->billing->title }}
                            </div>
                            <div class="text-[10px] text-slate-400">Total Tagihan: Rp
                                {{ number_format($pay->billing->amount, 0, ',', '.') }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <button wire:click="showDetail('{{ $pay->id }}')"
                                class="px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-bold hover:bg-brand-blue transition shadow-md">
    ...
    @if ($isModalOpen && $selectedPayment)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-sm p-4">
            <div
                class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]">

                <!-- Kolom Kiri: Bukti Gambar -->
                <div
                    class="w-full md:w-1/2 bg-slate-200 dark:bg-slate-900 flex items-center justify-center p-4 overflow-hidden relative">
                    ...
                    @endif
                    <div
                        class="absolute top-2 left-2 bg-black/50 text-white text-[10px] px-2 py-1 rounded backdrop-blur">
                        Bukti Mahasiswa</div>
                </div>
                ...
                <div class="w-full md:w-1/2 p-6 flex flex-col">
                    ...
                    <div class="space-y-4 flex-1 overflow-y-auto">
                        ...
                        <div class="bg-yellow-50 border-l-4 border-yellow-400 p-3 text-xs text-yellow-800">
                            <strong>SOP Verifikasi:</strong><br>
                            Jangan hanya melihat gambar! Cek <strong>Mutasi Rekening Koran</strong>. Pastikan uang
                            sejumlah tersebut benar-benar masuk pada tanggal yang tertera.
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <p class="font-semibold text-slate-800 dark:text-white">
                                    {{ $selectedPayment->billing->student->user->name }}</p>
                            </div>
                            <div>
                                <p class="text-slate-400 text-xs uppercase font-bold">Tanggal Transfer</p>
                                <p class="font-semibold text-slate-800 dark:text-white">
                                    {{ $selectedPayment->payment_date->format('d F Y') }}</p>
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <p class="text-3xl font-black text-green-700 dark:text-green-400">Rp
                                {{ number_format($selectedPayment->amount_paid, 0, ',', '.') }}</p>
                        </div>
                        ...
                        @if ($selectedPayment->status == 'PENDING')
                            <div class="pt-4 border-t border-slate-100 dark:border-slate-700">
                                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Catatan
                                    (Jika Ditolak)</label>
                                <textarea wire:model="rejection_note" rows="2"
                                    class="w-full rounded-lg border-slate-300 text-sm dark:bg-slate-700 dark:text-white"
                                    placeholder="Contoh: Bukti buram, uang belum masuk mutasi..."></textarea>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="pt-6 mt-auto flex justify-end gap-3 border-t border-slate-100 dark:border-slate-700">
                        <button wire:click="$set('isModalOpen', false)"
                            class="px-4 py-2 rounded-lg text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700">Tutup</button>

                        @if ($selectedPayment->status == 'PENDING')
                            <button wire:click="reject" wire:confirm="Yakin tolak pembayaran ini?"
                                class="px-4 py-2 rounded-lg border border-red-200 text-red-600 font-bold hover:bg-red-50">
                                Tolak
                            </button>
                            <button wire:click="verify" wire:confirm="Saya sudah cek mutasi bank dan uangnya ADA?"
                                class="px-6 py-2 rounded-lg bg-green-600 text-white font-bold hover:bg-green-700 shadow-lg shadow-green-500/30">
                                TERIMA (VERIFIKASI)
                            </button>
                        @endif
                    </div>
