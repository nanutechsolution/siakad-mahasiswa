<div>
    ...
    <div class="flex flex-col md:flex-row justify-between mb-6 gap-4">
        <div class="flex gap-2">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari Mahasiswa / Tagihan..."
        ...
        <table class="w-full text-left text-sm">
            <thead class="bg-slate-50 dark:bg-slate-700 text-slate-500 uppercase text-xs font-bold">
                <tr>
                    ...
                    <th class="px-6 py-4">Nominal Awal</th>
            ...
            {{ $bill->category == 'SPP' ? 'bg-blue-50 text-blue-600 border-blue-200' : '' }}
            {{ $bill->category == 'GEDUNG' ? 'bg-purple-50 text-purple-600 border-purple-200' : '' }}
            {{ $bill->category == 'SERAGAM' ? 'bg-orange-50 text-orange-600 border-orange-200' : '' }}
            {{ $bill->category == 'LAINNYA' ? 'bg-slate-50 text-slate-600 border-slate-200' : '' }}">
                                    {{ $bill->category }}
                                </span>
                                <div class="font-medium text-slate-800 dark:text-slate-200">{{ $bill->title }}</div>
                            </div>
                            <div class="text-xs text-slate-400 mt-1">{{ Str::limit($bill->description, 30) }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            @if ($bill->status == 'PAID')
                                <span
                                    class="bg-green-100 text-green-800 text-[10px] font-bold px-2 py-1 rounded-full">LUNAS</span>
                            @elseif($bill->status == 'PARTIAL')
                                <span
                                    class="bg-yellow-100 text-yellow-800 text-[10px] font-bold px-2 py-1 rounded-full">CICIL</span>
                            @else
                                <span
                                    class="bg-red-100 text-red-800 text-[10px] font-bold px-2 py-1 rounded-full">BELUM</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right">
                            <button wire:click="showDetail('{{ $bill->id }}')"
                                class="text-blue-600 hover:underline text-xs font-bold">Detail & Sisa</button>
                        </td>
                    </tr>
                @empty
                    <tr>
        ...
        <div
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg my-8">
                ...
                <form wire:submit.prevent="store" class="p-6 space-y-5">
                    <div>
                        <label class="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Target
                            Penerima</label>
                        <div class="flex gap-4 mb-3">
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="radio" wire:model.live="target_type" value="prodi"
                                    class="text-brand-blue focus:ring-brand-blue">
                                <span class="ml-2 text-sm">Per Prodi</span>
                            </label>
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="radio" wire:model.live="target_type" value="angkatan"
                                    class="text-brand-blue focus:ring-brand-blue">
                                <span class="ml-2 text-sm">Per Angkatan</span>
                            </label>
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="radio" wire:model.live="target_type" value="individual"
                                    class="text-brand-blue focus:ring-brand-blue">
                        ...
                        @if ($target_type == 'prodi')
                            <select wire:model="prodi_id"
                                class="w-full rounded-lg border-slate-300 text-sm dark:bg-slate-700 dark:text-white">
                        ...
                        @elseif($target_type == 'angkatan')
                            <input wire:model="entry_year" type="number" placeholder="Tahun Angkatan (Misal: 2024)"
                                class="w-full rounded-lg border-slate-300 text-sm dark:bg-slate-700 dark:text-white">
                        @elseif($target_type == 'individual')
                            <input wire:model="specific_student_nim" type="text" placeholder="Masukkan NIM Mahasiswa"
                                class="w-full rounded-lg border-slate-300 text-sm dark:bg-slate-700 dark:text-white">
                        @endif
                    </div>
                    ...
                    <div class="space-y-3 border-t border-slate-100 pt-4 dark:border-slate-700">
                        ...
                        <div>
                            ...
                            <select wire:model="category"
                                ...
                                <option value="GEDUNG">Uang Gedung / Pembangunan</option>
                                <option value="SERAGAM">Seragam / Almamater</option>
                                <option value="WISUDA">Biaya Wisuda</option>
                                <option value="KKN">Biaya KKN / Praktek</option>
                                <option value="LAINNYA">Lainnya</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-bold mb-1 text-slate-700 dark:text-slate-300">Judul
                                Tagihan</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-bold mb-1 text-slate-700 dark:text-slate-300">Jatuh
                                    Tempo</label>
                    ...
                    <div class="pt-4 flex justify-end gap-2 border-t border-slate-100 dark:border-slate-700">
                        <button type="button" wire:click="$set('isModalOpen', false)"
                            class="px-4 py-2 rounded text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-700">Batal</button>
                        <button type="submit"
                            class="px-6 py-2 rounded bg-brand-blue text-white hover:bg-blue-800 shadow-lg font-bold">
                            Simpan Tagihan
                        </button>
                    </div>
                </form>
    ...
    <!-- MODAL DETAIL (PERBAIKAN: MENAMPILKAN SISA TAGIHAN) -->
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden">
                <div class="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    ...
                    <button wire:click="$set('isDetailModalOpen', false)"
                        class="text-slate-400 hover:text-red-500">&times;</button>
                </div>

                <div class="p-6 space-y-6">
                    <!-- Info Utama (REVISI: Menampilkan Total, Dibayar, Sisa) -->
                    <div
                        class="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-100 dark:border-slate-700">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <p class="text-xs text-slate-500 uppercase font-bold">Mahasiswa</p>
                                <p class="font-bold text-slate-900 dark:text-white">
                                    {{ $selectedBillingDetail->student->user->name }}</p>
                                <p class="text-xs text-slate-400 font-mono">{{ $selectedBillingDetail->student->nim }}
                                </p>
                            </div>
                            <div class="text-right">
                                <span
                                    class="bg-blue-100 text-blue-800 text-[10px] font-bold px-2 py-1 rounded-full">{{ $selectedBillingDetail->title }}</span>
                            </div>
                        </div>

                        <div class="grid grid-cols-3 gap-4 border-t border-slate-200 dark:border-slate-600 pt-4">
                            <div>
                                <p class="text-[10px] text-slate-500 uppercase font-bold">Total Tagihan</p>
                                <p class="font-mono font-bold text-slate-700 dark:text-slate-300">Rp
                                    {{ number_format($selectedBillingDetail->amount, 0, ',', '.') }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-green-600 uppercase font-bold">Sudah Dibayar</p>
                                <p class="font-mono font-bold text-green-600">Rp
                                    {{ number_format($total_paid, 0, ',', '.') }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] text-red-500 uppercase font-bold">Sisa Tagihan</p>
                                <p class="font-mono font-black text-xl text-red-500">Rp
                                    {{ number_format($remaining_balance, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Riwayat Pembayaran -->
                    <div>
                        <h4 class="font-bold text-slate-800 dark:text-white mb-3">Riwayat Transaksi</h4>
                        <div class="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
                            <table class="w-full text-sm text-left">
                                <thead
                                    class="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs uppercase font-bold">
                                    <tr>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                                    @forelse($selectedBillingDetail->payments as $pay)
                                        <tr>
                                            ...
                                            <td class="px-4 py-3 font-mono">Rp
                                            ...
                                            <td class="px-4 py-3 text-center">
                                                ...
                                                @else
                                                    <span class="text-yellow-600 font-bold text-xs">Menunggu</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @empty
                ...
                <div class="p-6 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                    <button wire:click="$set('isDetailModalOpen', false)"
                        class="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-white rounded-lg font-bold hover:bg-slate-300">Tutup</button>
                </div>
            </div>
        </div>
    @endif
