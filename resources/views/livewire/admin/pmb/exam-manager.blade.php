        ...
        <div
            ...
            <div
                ...
                <form wire:submit.prevent="store" class="p-6 space-y-6">
                    ...
                    <!-- Tombol Batal -->
                    <button type="button" wire:click="$set('isModalOpen', false)" class="px-6 py-2.5 rounded-xl text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-700 font-bold transition">
                        Batal
                    </button>
                    
                    <div class="flex gap-2">
                        <!-- Tombol Simpan & Tambah Lagi (Hanya muncul saat Create) -->
                        @if(!$isEditMode)
                            <button type="button" 
                                    wire:click="store(false)" 
                                    wire:loading.attr="disabled"
                                    class="px-6 py-2.5 rounded-xl bg-indigo-50 text-indigo-700 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-300 font-bold transition flex items-center gap-2">
                                <span wire:loading.remove wire:target="store(false)">+ Simpan & Buat Lagi</span>
                                <span wire:loading wire:target="store(false)">Menyimpan...</span>
                            </button>
                        @endif
                        ...
                        <button type="button"
                            ...
                            <span wire:loading.remove wire:target="store(true)">{{ $isEditMode ? 'Simpan Perubahan' : 'Simpan & Tutup' }}</span>
                            <span wire:loading wire:target="store(true)">Menyimpan...</span>
                            <svg wire:loading wire:target="store(true)" class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                        </button>
                    </div>
                </div>
